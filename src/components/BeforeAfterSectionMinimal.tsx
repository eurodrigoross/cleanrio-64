  ...
  return <section className="section-container">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto">
          {/* Slider de imagens */}
          <div className="mb-8">
            <div ref={containerRef} className="relative h-96 md:h-[500px] rounded-2xl overflow-hidden cursor-ew-resize shadow-xl bg-neutral-100" onMouseMove={handleMouseMove}>
              {/* Imagem "depois" (fundo) */}
              <div className="absolute inset-0">
                <img src={slides[activeSlide].image} alt={`${slides[activeSlide].title} - depois da limpeza`} className="w-full h-full object-cover" />
              ...
              clipPath: `inset(0 ${100 - position}% 0 0)`
                ...
                <img src={slides[activeSlide].image} alt={`${slides[activeSlide].title} - antes da limpeza`} className="w-full h-full object-cover filter brightness-50 sepia-[0.3]" />
              ...
              {/* Linha divisória */}
              <div className="absolute top-0 bottom-0 w-1 bg-white shadow-lg z-10" style={{
              left: `${position}%`
            }}>
          ...
          <div className="flex justify-center gap-2 mb-12">
            {slides.map((_, index) => <button key={index} onClick={() => setActiveSlide(index)} className={`w-3 h-3 rounded-full transition-colors duration-200 ${activeSlide === index ? 'bg-primary' : 'bg-neutral-300'}`} />)}
          </div>
